...
class BatteryViewModel(application: Application) : AndroidViewModel(application) {
     ...
     */
    private fun adjustUpdateInterval(currentPrediction: ImprovedBatteryCycleEstimator.ShutdownPrediction) {
        val drainRate = estimator.getWeightedDrainRate()

        val newInterval = when {
            currentPrediction.minutesLeft < 10.0 -> CRITICAL_UPDATE_INTERVAL
            drainRate > 3.0 -> 2000L  // Battery draining fast, check more often
            drainRate > 1.5 -> 3000L  // Moderate drain, check every 3 sec
            currentPrediction.confidence == ImprovedBatteryCycleEstimator.PredictionConfidence.LOW -> LOW_CONFIDENCE_UPDATE_INTERVAL
            else -> DEFAULT_UPDATE_INTERVAL
        }

        val safeInterval = newInterval.coerceAtLeast(500)  // Ensure at least 500ms interval
